...
## Initialization
To let user to use custom `zlibwapi.dll`, ZLibWrapper internally loads zlib dynamically (using `LoadLibrary` and `GetProcAddress`).

Personal recommendation is case 2, since it provides newer version of zlib while convenient.

### Case 1 : No Explicit Initialzation
...
WARNING: Architecture of `zlibwapi.dll` must be matched with .Net Framework! 
...
using (Adler32Stream adler = new Adler32Stream(fs))
{
    ...
    Console.WriteLine("0x" + adler.Digest.ToString("X8")); // 0x000D0007

    bin = new byte[] { 0x04, 0x05, 0x06 };
    adler.Write(bin, 0, bin.Length);
    Console.WriteLine("0x" + adler.Digest.ToString("X8")); // 0x003E0016
}
```
